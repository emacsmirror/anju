...
Anju makes these following modifications to the main menu bar *File* menu:
...
Anju organizes its customization of the main menu bar via the customizable hook variable ~anju-reconfigure-main-menu-hook~.

~anju-reconfigure-main-menu-hook~ is a list of hook functions,  where each function is designed to modify a menu of the main menu bar (e.g. "File", "Edit", "Options", etc.). This hook is run in the command ~anju-init~.
...
Note that Anju's implementation of hook functions each only affecting a menu in the main menu bar is by convention.
...
~anju-main-menu--reconfigure-file~ is a hook function which modifies the *File* menu in the main menu bar.

By default this function is in ~anju-reconfigure-main-menu-hook~. Enabling or disabling this function can be done via the checkbox interface provided by the ~customize-variable~ command.

~anju-main-menu--reconfigure-file~ will read the customizable variable ~anju-file-menu-replace-make-frame-on~ (default ~t~). If ~anju-file-menu-replace-make-frame-on~ is non-nil, then the following menu items are replaced with more context-specific behavior:
...
~anju-main-menu--reconfigure-options~ is a hook function which modifies the *Options* menu in the main menu bar.
...
# #+VINDEX: anju-main-menu--reconfigure-edit-menu
...
~anju-main-menu--reconfigure-textmode~ is a hook function which reorganizes the *Text* menu in the main menu bar.
...
~anju-main-menu--reconfigure-bookmarks~ is a hook function which adds the *Bookmarks* menu in the main menu bar.
...
~anju-main-menu--reconfigure-help~ is a hook function which reorganizes the *Help* menu in the main menu bar.

By default this function is in ~anju-reconfigure-main-menu-hook~. Enabling or disabling this function can be done via the checkbox interface provided by the ~customize-variable~ command.

If removing the Emacs tutorial menu items from the *Help* menu is desired, set the customizable variable ~anju-help-menu-remove-emacs-tutorial~ to ~t~.
...
~anju-main-menu--reconfigure-imenu~ is a hook function which adds the *Index* menu in the main menu bar.
