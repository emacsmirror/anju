...
If the context menu is raised on a headline, then commands related to TODO state, clocking, and promotion are offered.
...
If the context menu is raised on a list item, then commands related to promotion and checkbox operations are provided.
...
On selected text, the context menu will have the menu item “Link…” to run the command ~org-insert-link~. From there you can enter a link, typically copied into the system clipboard.
...
- Toggle Images :: toggles display of images when running in GUI.
- Show Markup :: reveals or hides display of Org markup.
...
# TODO: Write Makefile Context Menu
...
For certain modes that support Imenu ([[info:emacs#Imenu]]), Anju will add an *Index* menu to the main menu bar via the function  ~imenu-add-menubar-index~.
...
Anju makes the following modifications to the main menu bar *Tools* menu:

- Adds the “Macro Recorder” sub-menu :: A menu of commands pertaining to keyboard macros ([[info:emacs#Keyboard Macros]]).
...
Enabling ~anju-main-menu--reconfigure-imenu~ can be done using the checkbox interface provided by ~customize-variable~ in modifying ~anju-reconfigure-main-menu-hook~.
...
# TODO: Rewrite Tools Menu Customization

The hook function ~anju-main-menu--reconfigure-tools~ in ~anju-reconfigure-main-menu-hook~ modifies the *Tools* menu in the main menu bar.

Enabling ~anju-main-menu--reconfigure-tools~ can be done using the checkbox interface provided by ~customize-variable~ in modifying ~anju-reconfigure-main-menu-hook~.
