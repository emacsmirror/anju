...
It is recommended that the ~org-mouse~ package be loaded when using
...
[[file:images/anju-context-menu-org-checkbox.png]]
...
If hideshow ([[info:emacs#Hideshow][emacs#Hideshow]]) support is enabled, then a sub-menu “Hide/Show” is provided.
...
Note that the Emacs default context menu for Elisp supports Xref ([[info:emacs#Xref][emacs#Xref]]) navigation as well as help (~describe-function~) and Info lookup as shown below.
...
To better understand Edebug's command set, a close reading of its manual is recommended ([[info:elisp#Edebug][elisp#Edebug]]). 
...
ERT is a built-in package for Emacs that supports automated testing of Elisp ([[info:ert#Top][ert#Top]]). Anju provides a convenience command to run an ~ert-deftest~ if the point is within its declaration. The screenshot below shows and example of running the ERT test ~test-foo~.
...
Anju enhances the main menu bar of Emacs by making opinionated changes to its existing menus as well as adding several new ones. These changes are configurable per menu in the main menu (e.g. "File", "Edit", "Options", etc.). For more detail, see [[#main-menu-customization][Main Menu Customization]].


*** File Menu Features
#+CINDEX: File Menu Features

Anju makes the following modifications to the main menu bar *File* menu:
...
Anju makes the presumption that most Emacs users do not use the CUA mode configuration in the main menu bar *Options* menu. As such, the “Cut/Paste with C-x/C-c/C-v (CUA Mode)” menu item is removed.
...
If the major mode of the current buffer is derived from ~text-mode~, then a *Text* menu is displayed in the main menu bar.
...
For certain modes that support Imenu ([[info:emacs#Imenu][emacs#Imenu]]), Anju will add an *Index* menu to the main menu bar via the function  ~imenu-add-menubar-index~.
...
The context predicate can be embedded in the menu item definition itself. See more about this with the ~:visible~ and ~:active~ keywords for the macro ~easy-menu-define~ ([[info:elisp#Easy Menu][elisp#Easy Menu]]).
...
Users are free to implement their own hook functions to add to ~anju-reconfigure-main-menu-hook~. Users who wish to do this should familiarize themselves with menu keymaps ([[info:elisp#Menu Keymaps][elisp#Menu Keymaps]]) and the Easy Menu ([[info:elisp#Easy Menu][elisp#Easy Menu]]) library.

Note that Anju's implementation of hook functions to each affect a specific menu in the main menu bar is by convention.

For more info on hook variables see [[info:emacs#Hooks][emacs#Hooks]].
...
The hook function ~anju-main-menu--reconfigure-file~ in ~anju-reconfigure-main-menu-hook~ modifies the *File* menu in the main menu bar.

Enabling ~anju-main-menu--reconfigure-file~ can be done using the checkbox interface provided by ~customize-variable~ when modifying ~anju-reconfigure-main-menu-hook~.
...
The hook function ~anju-main-menu--reconfigure-edit~ in ~anju-reconfigure-main-menu-hook~ modifies the *Edit* menu in the main menu bar.

Enabling ~anju-main-menu--reconfigure-edit~ can be done using the checkbox interface provided by ~customize-variable~ when modifying ~anju-reconfigure-main-menu-hook~.
...
The hook function ~anju-main-menu--reconfigure-options~ in ~anju-reconfigure-main-menu-hook~ modifies the *Options* menu in the main menu bar.

Enabling ~anju-main-menu--reconfigure-options~ can be done using the checkbox interface provided by ~customize-variable~ when modifying ~anju-reconfigure-main-menu-hook~.
...
The hook function ~anju-main-menu--reconfigure-text-mode~ in ~anju-reconfigure-main-menu-hook~ modifies the *Text* menu in the main menu bar.

Enabling ~anju-main-menu--reconfigure-text-mode~ can be done using the checkbox interface provided by ~customize-variable~ when modifying ~anju-reconfigure-main-menu-hook~.
...
The hook function ~anju-main-menu--reconfigure-bookmarks~ in ~anju-reconfigure-main-menu-hook~ adds the *Bookmarks* menu to the main menu bar.

Enabling ~anju-main-menu--reconfigure-bookmarks~ can be done using the checkbox interface provided by ~customize-variable~ when modifying ~anju-reconfigure-main-menu-hook~.
...
The hook function ~anju-main-menu--reconfigure-help~ in ~anju-reconfigure-main-menu-hook~ modifies the *Help* menu in the main menu bar.

Enabling ~anju-main-menu--reconfigure-help~ can be done using the checkbox interface provided by ~customize-variable~ in modifying ~anju-reconfigure-main-menu-hook~.
...
The hook function ~anju-main-menu--reconfigure-imenu~ in ~anju-reconfigure-main-menu-hook~ adds the *Index* menu to the main menu bar.

Enabling ~anju-main-menu--reconfigure-file~ can be done using the checkbox interface provided by ~customize-variable~ in modifying ~anju-reconfigure-main-menu-hook~.
...
For more info on Imenu see [[info:emacs#Imenu][emacs#Imenu]].
